...
//we are updating the age using "user" reference so it will not be impacted in the object which is refered by "myUser" reference variable
...
let newUser = structuredClone(user); //structuredClone method say give me the object of which you want to create the clone 
...
console.log('initial original object ==> ',customer); //1
console.log('--------------');
//Now we want to create copy:
let newCustomer = { ...customer }; //2
console.log('cloned netsed object ==>',newCustomer); //3
 ...
 * 
 * suppose tomorrow if i am creating a new customer and changing the entire address, then all the users will be updated with the new address. so thats the major bug here.
 * only nested will be impacted 
 * if you update the parent object property/level 1 property then original/initial object will not be updated 
 * 
...
console.log('cloned netsed updated object ==> ',newCustomer);//5
console.log('----------');
console.log('initial original object got updated which is a bug ==> ',customer);//6


console.log("=========== Soluation - structuredClone example ==================");
